export const vibiClerkAuthenticationSystem = {
  ...
  description: "User authentication system for aviation baggage interlining with multi-tenant organization support",
  longDescription: `Built a comprehensive user management system for ViBI (Virtual Baggage Interlining), an aviation industry platform that connects B2B clients. The system provides enterprise-grade authentication with complex organizational hierarchies and role-based access control.

The platform manages five distinct organization types (system owner and client types) with a sophisticated three-tier permission system: Organization Type → Department → Role. Each organization maintains complete data isolation while enabling secure inter-organizational baggage interlining operations.
...
Key achievement: Successfully implemented a scalable authentication architecture that handles complex aviation industry workflows while maintaining regulatory compliance and audit trails required for international baggage operations.`,
  ...
  challenges: [
    ...
    "Ensuring Global Data Protection Regulation (GDPR) compliance with audit trails and data isolation requirements",
  ...
  learnings: [
    ...
    "Global Data Protection Regulation (GDPR) compliance requirements and international compliance standards",
  ...
  features: [
    ...
    "SSO integration with custom authentication flows",
